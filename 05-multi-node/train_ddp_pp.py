...
def main():
    ...
    ddp_degree = ddp_mesh.size()
    ddp_rank = ddp_mesh.get_local_rank()
    ...
    if is_local_device0:
        ...
        print(f"[node{node_rank}] ddp_mesh: {ddp_mesh}")
    ...
    if is_local_device0: print(f"[GPU{node_rank}] Applied DDP")
    ...
    with maybe_enable_profiling(
        ...
        for epoch in range(start_epoch,total_epochs):
            ...
            if is_last_device: print(f"{datetime.now()} [node{node_rank}] Loss: {epoch_train_loss}\tLR: {current_lr}")
            lr_scheduler.step()
