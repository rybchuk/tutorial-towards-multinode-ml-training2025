        ...
        for epoch in range(start_epoch,total_epochs):
            ...
            for batch_idx, (model_input, target) in enumerate(dl_train): # data shape: [b,c,h,w]
                ...
                with record_function("## forward ##"):
                    print(f"\t[Batch {batch_idx}] Memory allocated, before forward: {memory_allocated(device=device)/1e9:.3f} GB")
                    model_output = model(model_input) 
                    print(f"\t[Batch {batch_idx}] Memory allocated, after forward: {memory_allocated(device=device)/1e9:.3f} GB")
                    loss = loss_fcn(model_output, target)
                
                # Backward
                with record_function("## backward ##"):
                    print(f"\t[Batch {batch_idx}] Memory allocated, before backward: {memory_allocated(device=device)/1e9:.3f} GB")
                    loss.backward()
                    print(f"\t[Batch {batch_idx}] Memory allocated, after backward: {memory_allocated(device=device)/1e9:.3f} GB")
                
                with record_function("## optimizer ##"):
                    optimizer.step()
                    print(f"\t[Batch {batch_idx}] Memory allocated, after optimizer: {memory_allocated(device=device)/1e9:.3f} GB")
